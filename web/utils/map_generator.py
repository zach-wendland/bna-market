...
def create_property_map(df, property_type):
    ...
    if df is None or df.empty:
        return (
            f"<div class='empty-state'>No {property_type} property data available for mapping</div>"
        )
    ...
    if df_filtered.empty:
        return (
            f"<div class='empty-state'>No {property_type} properties with valid coordinates</div>"
        )
    ...
    fig.update_traces(
        hovertemplate="%{hovertext}<extra></extra>", hovertext=df_filtered["hover_text"]
    )
...
def create_price_heatmap(df, property_type):
    ...
    if df is None or df.empty:
        return (
            f"<div class='empty-state'>No {property_type} property data available for heatmap</div>"
        )
    ...
    if property_type == "for-sale":
        ...
        df_filtered["hover_text"] = df_filtered.apply(
            lambda row: (
                f"<b>{row.get('address', 'N/A')}</b><br>" f"Price: ${row.get('price', 0):,.0f}"
            ),
            axis=1,
        )
        z_col = "price"
        ...
        df_filtered["hover_text"] = df_filtered.apply(
            lambda row: (
                f"<b>{row.get('address', 'N/A')}</b><br>" f"Rent: ${row.get('price', 0):,.0f}/mo"
            ),
            axis=1,
        )
        z_col = "price"
