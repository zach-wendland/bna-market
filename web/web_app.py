...
def dashboard():
    ...
    if not fred_metrics.empty:
        # Median price trend
        median_price_data = fred_metrics[fred_metrics["metric_name"] == "median_price"].sort_values(
            "date"
        )
        ...
        active_listings_data = fred_metrics[
            fred_metrics["metric_name"] == "active_listings"
        ].sort_values("date")
        ...
        # Employment trend
        employment_data = fred_metrics[
            fred_metrics["metric_name"] == "employment_non_farm"
        ].sort_values("date")
