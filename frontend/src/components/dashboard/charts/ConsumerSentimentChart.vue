...
const { chartColors, timeScaleOptions } = useChartConfig()

const chartData = computed(() => {
  if (!store.fredMetrics || store.fredMetrics.length === 0) return null

  // Filter for consumer sentiment
  const sentimentData = store.fredMetrics
    .filter(m => m.metricName === 'consumer_sentiment')
    .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())

  // Filter for building permits
  const permitsData = store.fredMetrics
    .filter(m => m.metricName === 'building_permits')
    .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())

  if (sentimentData.length === 0 && permitsData.length === 0) return null

  const labels = sentimentData.length > 0
    ? sentimentData.map(m => m.date)
    : permitsData.map(m => m.date)
  ...
  return {
    ...
    datasets: [
      ...(sentimentData.length > 0 ? [{
        ...
        data: sentimentData.map(m => m.value),
      ...
      ...(permitsData.length > 0 ? [{
        ...
        data: permitsData.map(m => m.value),
        borderColor: chartColors.wheat,
    ...
    tooltip: {
      ...
      callbacks: {
        label: (context: any) => {
          ...
          if (label.includes('Sentiment')) return `${label}: ${value.toFixed(1)}`
